...
for f in $lang/words.txt $lang/phones.txt $model $ali_dir/ali.1.gz $lang/oov.int; do
...
if [ $stage -le 0 ]; then
    echo '\n------Obtaining Phone level alignments----------\n'
...
if [ $stage -le 1 ]; then
  ...
  #rm $dir/phone_ctm.*.gz
  #sed -i '/SIL/d' $dir/phone_ctm
  #sed -i 's/_[^_]*$//g' $dir/phone_ctm

fi
#exit 1
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_word_ctm.JOB.log \
      set -o pipefail '&&' linear-to-nbest "ark:gunzip -c $ali_dir/ali.JOB.gz|" \
      "ark:utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt < $sdata/JOB/text |" \
      '' '' ark:- \| \
    ...
    if [ ! -f $lang/phones/align_lexicon.int ]; then
      echo "$0: $lang/phones/word_boundary.int do not exist: cannot align."
      exit 1;
    fi
    $cmd JOB=1:$nj $dir/log/get_word_ctm.JOB.log \
      set -o pipefail '&&' linear-to-nbest "ark:gunzip -c $ali_dir/ali.JOB.gz|" \
...
if [ $stage -le 1 ]; then
  ...
  rm $dir/ctm.*.gz
  #sed -i '/SIL/d' $dir/ctm
  #sed -i '/SIL\|ON\|MB\|FP\|BR\|IR\|WH/d' $dir/ctm
fi
