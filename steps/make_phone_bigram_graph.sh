...
stage=0
cmd=run.pl
N=2  # change N and P for non-trigram systems.
P=1
...
if [ $free_phone_loop -eq 1 ]; then
    cut -f1 -d ' ' $lang/phones.txt > $dir/phone_graph/train_phones.txt
    sed -i '1d' $dir/phone_graph/train_phones.txt
    sed -i 's/^/SIL /;s/$/ SIL/' $dir/phone_graph/train_phones.txt
    sed -i '/#/d' $dir/phone_graph/train_phones.txt

fi

if [ $stage -le 1 ]; then
  echo "$0: building ARPA LM"
  if [ $free_phone_loop -eq 0 ]; then
  ngram-count -text $dir/phone_graph/train_phones.txt -order 2  \
    -addsmooth1 1 -kndiscount2 -interpolate -lm $dir/phone_graph/arpa.gz
  else
      ngram-count -text $dir/phone_graph/train_phones.txt -order 1  \
          -lm $dir/phone_graph/arpa.gz
      N=2
      P=1
